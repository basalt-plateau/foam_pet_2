...
module builder_1::Boar_Game_Module {
	...
	const Ending_every_boar_Play_has_been_sold : u64 = 3;
	const Ending_cannot_exit_the_game_if_have_plays : u64 = 3;
	
	struct Boar_Play has key, drop {}
	
	struct Boar_Game has key, drop {
		boar_Plays_for_sale : u256,
	...
	public entry fun Build (
		...
		let boar_Boar_Game = Boar_Game {
			boar_Plays_for_sale : boar_Plays_for_sale,
	...
	public entry fun Exit_the_Boar_Game (consenter : & signer) acquires Boar_Game {
		...
		if (Boar_Team_Module::boar_Plays_score (boar_Team_at_index_ref) != 0) {
	...
	//
	//
	#[view] public fun Boar_Plays_For_Sale_Left () : u256 acquires Boar_Game {
		let sport = borrow_global<Boar_Game>(boar_Producer_position ());
		sport.boar_Plays_for_sale
	}
	public entry fun Buy_Boar_Plays_for_1_APT_each (
		consenter : & signer,
		...
		let sport = borrow_global<Boar_Game>(boar_Producer_position ());
		if (sport.boar_Plays_for_sale < amount_of_plays_to_buy) {
		...
		Boar_Team_Module::add_boar_Plays (boar_Team_at_index_ref, amount_of_plays_to_buy);	
	...
	public entry fun Throw_Boar_Play (
		...
		Boar_Team_Module::subtract_boar_Plays (boar_Team_from_ref, 1);	

		let boar_Team_to_ref = vector::borrow_mut (boar_Teams, index_of_boar_Team_to);		
		Boar_Team_Module::add_boar_Plays (boar_Team_to_ref, 1);		
	}
	...
	#[view]
	public fun Boar_Plays_Score (boar_Team_address : address) : u256 acquires Boar_Game {
		...
		Boar_Team_Module::boar_Plays_score (boar_Team_at_index_ref)
	}
