	...
	*/
	trucks [1] = build_truck ({
		freight: {
			...
			leaves: [],
			add (packet) {
				const { tome, epithet } = packet;
				
				const leaf_barcode = trucks [1].freight.next_leaf_barcode;
				trucks [1].freight.next_leaf_barcode++;
				
				trucks [1].freight.leaves.push ({
				...
				return {
					exit () {
						/*
							Find the barcode and then delete the modal.
						*/
					}
				}
			}
